...
import Data_Analysis as function
import Data_Preprocessing as preprocessing_function
import Home_Page 
import base64

st.set_page_config(page_icon="✍️", page_title="AutoGraph")
...
st.markdown("<h1 style='text-align: center;'>Welcome to AutoGraph</h1>", unsafe_allow_html=True)
st.write("")


selected = option_menu(
    menu_title=None,
    options=['Home', 'Data Exploration', 'Data Processing'],
...
if uploaded_file is not None:
    ...
    if use_example_data:
        ...
        st.warning("Please upload a dataset or choose to use the example dataset.")
    ...
    if selected == 'Data Processing':
        ...
        if missing_count.any():
            ...
            if selected_missing_option == "Remove Rows in Selected Columns":
                ...
                numerical_columns = st.session_state.new_df.select_dtypes(include=['integer']).columns
                columns_with_missing = st.session_state.new_df.columns[st.session_state.new_df.isna().any()]
                numerical_columns_with_missing = list(set(numerical_columns).intersection(columns_with_missing))
